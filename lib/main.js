...
const turfBboxPoly = require('@turf/bbox-polygon').default;
const turfTruncate = require('@turf/truncate').default;
const turfDestination = require('@turf/destination').default;
...
class MapboxCircle {
    ...
    _updateCircle() {
        ...
        if (!(this._centerDragActive && radius < 10000)) {
            ...
            this._circle = turfCircle(center, radius, {
                'steps': steps,
                'units': unit,
        ...
        if (this.options.editable) {
            this._handles = [
                turfDestination(center, radius, 25, {'units': unit})
            ];
        }
    ...
    _getCenterHandleGeoJSON() {
        if (this._centerDragActive && this.radius < 10000) {
            return turfHelpers.featureCollection([turfDestination(this.center, this.radius, 0, {'units': 'meters'})]);
        } else {
            return turfHelpers.featureCollection([turfDestination(this.center, this.radius, 0, {'units': 'meters'}), this._circle]);
        }
    }
    ...
    _highlightHandles(layerId, cursor) {
        ...
        this.map.setPaintProperty(layerId, 'circle-stroke-width', this.options.strokeWeight + 1);
    ...
    _resetHandles(layerId) {
        ...
        this.map.setPaintProperty(layerId, 'circle-stroke-width', this.options.strokeWeight);
    ...
    _onCenterHandleMouseMove(event) {
        ...
        const mousePoint = turfTruncate(turfHelpers.point(this.map.unproject(event.point).toArray()), {
            'precision': 6
        });
        const newCenterFeature = turfDestination(mousePoint.geometry.coordinates, this.radius, 180, {'units': 'meters'});

        // check center in radius
        if(this._circle.properties.based_center && this._circle.properties.based_radius) {

            const steps = this.options.refineStroke ? Math.max((Math.sqrt(Math.trunc(radius * 0.25)) * zoom ^ 2), 64) : 64;
            const unit = 'meters';
            const basedCircle = turfCircle(this._circle.properties.based_center, this._circle.properties.based_radius, {
                'steps': steps,
                'units': unit
            });

            // Not in circle, stop dragging
            if(!turfBooleanPointInPolygon(newCenterFeature.geometry.coordinates, basedCircle)) {
                return;
            }
        }

        this.center = newCenterFeature.geometry.coordinates;
    }
    ...
    _onCenterHandleMouseUpOrMapMouseOut(event) {
        ...
        if (event.type === 'mouseout' && event.originalEvent && event.originalEvent.toElement) {
    ...
    _getRadiusHandleCursorStyle(event) {
        const bearing = turfBearing(event.lngLat.toArray(), this._currentCenterLngLat, {'final': true});
    ...
    _onRadiusHandlesMouseMove(event) {
        ...
        this.radius = Math.round(turfDistance(this.center, mousePoint, {'units': 'meters'}));
    }
    ...
    _getCircleFillLayer() {
        ...
        if(!!this.options.fillPattern) {
            paintOpts['fill-pattern'] = this.options.fillPattern;
        }
    ...
    _getCenterHandleStrokeLayer() {
        if (this._centerDragActive && this.radius < 10000) {
            ...
            const horizontalPixelsPerMeter = x / turfDistance(
                this.map.unproject([0, y]).toArray(), this.map.unproject([x, y]).toArray(), {'units': 'meters'});
            return {
     ...
     */
    getBounds() {
        const bboxPolyCoordinates = turfTruncate(turfBboxPoly(turfBbox(this._circle)), {
            'precision': 6
